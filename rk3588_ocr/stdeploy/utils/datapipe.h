...
class DataPipe {
public:
    DataPipe(std::string name, int max_len, int discard_mode = 2) : name_(name), max_size_(max_len),
                                                                    discard_mode_(discard_mode), stop_flag_(false) {};
...
#if PIPE_INFO
                STDEPLOY_INFO("%s pipe full",name_);
#endif
                if (discard_mode_ == 0) {
                    data_que_.pop_front();
